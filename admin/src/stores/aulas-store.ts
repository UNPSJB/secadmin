...
export const useAulasStore = defineStore('aulas', {
  ...
  actions: {
    async guardarAula(codigoAula: string, capacidad: number, localidad: any, direccion: string) {
    ...
    async actualizarAula(aulaId: string, codigo_aula: string, capacidad: number, localidad: any, direccion: string) {
